...
import dataPaths.ALUControler;
import dataPaths.ALUnit;
import dataPaths.Controler;
import dataPaths.Memory;
import dataPaths.RegisterData;
		...
		if (controler.branch == 1) 
		{
			if (controler.bne == 1 && !ALU.zero) 
			{
				programCounter.value = Converter.convertToBinary
					...
					(Integer.parseInt(programCounter.value, 2)- 
					 Integer.parseInt(signExtend
							 (HelperMethods.insFromXToY(instruction,0, 15)),2)), 8);
		...
		if (controler.jump == 1) 
		{
			if (controler.jumpToReturnAddress != 1) 
			{
				programCounter.value = Converter.convertToBinary
						(Integer.parseInt(programCounter.value, 2)- 
						(Integer.parseInt(programCounter.value, 2)- 
						 Integer.parseInt(HelperMethods.insFromXToY
								 (instruction, 0,25), 2)), 8);
			}
		}
